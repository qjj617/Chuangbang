...
import chuangbang.task.LoadLocalBigImgTask;
import chuangbang.utils.ImageCache;
import chuangbang.view.PhotoView;
...
public class ShowBigImage extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (uri != null && new File(uri.getPath()).exists()) {
			...
			// "Bearer "+EMChatConfig.getInstance().AccessToken());
	...
	private void downloadImage(final String remoteFilePath,
		...
		final CloudOperationCallback callback = new CloudOperationCallback() {
			...
			public void onError(String msg) {
				Log.i("我的TAG","### offline file transfer error:" + msg);
			...
			public void onProgress(final int progress) {
				Log.i("我的TAG " ,"ease Progress:"+ progress);
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
